...
def make_dataset():
    ...
    companies_tickers_list = ["MSFT", "NVDA", "AAPL", "AMZN", "AMD"]
    ...
    #     if company == "alphabet":
    #         companies_tickers_list.append("AMD")
    #     if company == "nvidia":
    ...
    if financial_data:
        for company_ticker in companies_tickers_list:
            for indicator in financial_indicators:
                data = pd.read_csv('./dataset/data/' + company_ticker + '/' + company_ticker + '_' + indicator + '.csv')
    ...
    # bonds interest
    data = pd.read_csv('./dataset/data/InterestRatesBonds/InterestRatesBonds.csv')
    data["Date"] = data["Year"].astype(str) + '-' + data["Month"].astype(str) + '-' + data["Day"].astype(str)
    data["Date"] = pd.to_datetime(data["Date"], format='%Y-%m-%d')
    ...
    #CPI
    data = pd.read_csv('./dataset/data/CPI/CPI.csv')
    ...
    #money supply
    data = pd.read_csv('./dataset/data/MoneySupply/MoneySupply.csv')
    data["Date"] = data["Year"].astype(str) + '-' + data["Month"].astype(str)
    data["Date"] = pd.to_datetime(data["Date"], format='%Y-%m')
    ...
    print('go to dataset and check if the last row does not contain empty prices - the session didnt end')
